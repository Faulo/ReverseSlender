...
namespace SDFr
{
    ...
    public class SDFBaker : AVolumeBaker<SDFVolume, SDFData>
    {
        ...
        public void LogDistances()
        {
            ...
            Vector3Int dim = new(sdfData.sdfTexture.width, sdfData.sdfTexture.height, sdfData.sdfTexture.depth);
            string log = $"{name}  Dimension: {dim}  MaxLength: {sdfData.maxDistance} Mag: {bounds.size.magnitude}\n";
        ...
        private void BakeComplete(SDFVolume sdfVolume, float[] distances, float maxDistance, object passthrough)
        {
            ...
            {
                string suggestedName = "sdfData_" + name;
            ...
            Texture3D newTex = new(sdfData.dimensions.x, sdfData.dimensions.y, sdfData.dimensions.z, TextureFormat.RHalf, mipmaps)
            {
        ...
        public void BakeHalfSizeTest()
        {
            ...
            // Save sdf
            string suggestedName = "sdfData_" + name + "_HalfBake";
            ...
            // Create Texture3D and set name to filename of sdfData
            Texture3D newTex = new(sdfmip.dimensions.x, sdfmip.dimensions.y, sdfmip.dimensions.z, TextureFormat.RHalf, false)
            {
                name = System.IO.Path.GetFileNameWithoutExtension(path)
            };
            newTex.SetPixels(colorBuffer);
